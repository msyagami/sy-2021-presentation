...
export async function getAllPostsForBlog(preview) {
  const data = await fetchAPI(
    `
    query Posts($where: JSON){
      posts(sort: "date:desc", limit: 25, where: $where) {
        title
        slug
        excerpt
        date
...
export async function getProjectAndMoreProjects(slug, preview) {
  ...
  query ProjectBySlug($where: JSON, $where_ne: JSON) {
    ...
    moreProjects: projects(sort: "date:desc", limit: 2, where: $where_ne) {
      title
      slug
      excerpt
      date
...
export async function getAllProjectsForProjPage(preview) {
  const data = await fetchAPI(
    `
    query Projects($where: JSON){
      projects(sort: "date:desc", limit: 25, where: $where) {
        title
        slug
        excerpt
        date
